...
final class Store: ObservableObject {
    @Published var entries: [FastSessionEntry] = []
    ...
    static func appSupportDirectory() -> URL {
        ...
        let dir = base.appendingPathComponent("FastingWindow", isDirectory: true)
    ...
    func load() {
        if let data = try? Data(contentsOf: entriesURL),
           let decoded = try? JSONDecoder().decode([FastSessionEntry].self, from: data) {
            entries = decoded
        } else {
    ...
    static func seedEntries() -> [FastSessionEntry] {
        [
            FastSessionEntry(title: "Overnight fast", value: Double(1))
        ]
    }
    ...
    @discardableResult
    func add(_ entry: FastSessionEntry) -> Bool {
    ...
    func update(_ entry: FastSessionEntry) {
    ...
    func delete(_ entry: FastSessionEntry) {
