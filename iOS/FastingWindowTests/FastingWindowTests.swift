import XCTest
@testable import FastingWindow

@MainActor
final class FastingWindowTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        let added = store.add(FastSessionEntry(title: "Test entry"))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        while store.entries.count < Store.freeEntryLimit {
            _ = store.add(FastSessionEntry(title: "Filler"))
        }
        let added = store.add(FastSessionEntry(title: "Overflow"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        let a = FastSessionEntry(title: "A")
        let b = FastSessionEntry(title: "B")
        store.entries = [a, b]
